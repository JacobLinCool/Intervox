...
| Interpret path | Mic → 24 kHz PCM16 → WebSocket uplink → translated audio delta → jitter buffer → resample 48 kHz → limiter → ring. |
| Interpret original-voice mix | Interpret path plus delayed original mixed under translated audio when `original_voice_percent > 0`. |
...
| Silence mode enforcement | OpenAI session torn down in Silence and PassThrough; no original leakage in Interpret. |
...
- [x] Implement Interpret delayed original path when original voice volume is above 0%.
...
- [x] Prevent original mic leakage in Interpret mode.
